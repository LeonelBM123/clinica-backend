...
from .serializers import PacienteSerializer, CitaMedicaListSerializer, CitaMedicaDetalleSerializer


class PacientesPorGrupoView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request):
        grupo = getattr(request.user, 'grupo', None)
        if not grupo:
            return Response({"error": "No se encontró el grupo del usuario."}, status=400)

        pacientes = Paciente.objects.filter(grupo=grupo)
        serializer = PacienteSerializer(pacientes, many=True)
        return Response(serializer.data)


class CitasPorPacienteView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, paciente_id):
        grupo = getattr(request.user, 'grupo', None)
        citas = Cita_Medica.objects.filter(paciente__id=paciente_id, grupo=grupo)
